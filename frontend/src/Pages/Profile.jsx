...
export default function Profile() {
  ...
  const completion = useMemo(() => {
    ...
    const values = [
      ...
      profile.matricule,
      addressLabel(profile) !== "Non renseignée" ? true : null,
      profile.coordonnees_bancaires?.iban_masque || null,
      profile.contact_urgence?.nom || null,
      getIdentityDocument(profile) ? true : null,
    ];
  ...
  const identityDocument = getIdentityDocument(profile);
  const residenceDocument = getResidenceDocument(profile);
  const bank = profile.coordonnees_bancaires || null;
    ...
    <div className="profile-page">
      ...
      <section className="profile-identity-card">
        <div className="profile-avatar-container">
          ...
          >
            {uploadingPhoto ? (
              <span className="profile-photo-loader" />
            ) : (
              <Camera size={18} />
            )}
          </button>
        ...
        <div className="profile-identity-main">
          ...
          <div className="profile-badges">
            <span className="profile-contract-badge">
              {contractLabel(profile.type_contrat)}
            </span>
        ...
        <div className="profile-completion">
          ...
          <div
            ...
            aria-label={`Dossier complété à ${completion} %`}
          >
            <span
              style={{
                width: `${completion}%`,
            ...
            <div className="profile-card-grid">
              ...
              >
                ...
                <EditableRow
                  ...
                  display={formatDate(profile.date_naissance)}
                  editing={isEditing}
                  onChange={handleChange}
                />
            ...
            <div className="profile-card-grid">
              ...
              >
                <DataRow
                  ...
                  value={
                    identityDocument
                      ? documentStatus(identityDocument)
                      : "Non renseignée"
                  }
                />
                ...
                <DataRow
                  label="N° pièce"
                  value={identityDocument?.numero}
                />

                <DataRow
                  label="Expiration pièce"
                  value={
                    identityDocument?.date_expiration
                      ? formatDate(
                          identityDocument.date_expiration
                        )
                      : "Non renseignée"
                ...
                <DataRow
                  ...
                  value={
                    residenceDocument
                      ? documentStatus(residenceDocument)
                      : "Non renseigné"
                  }
                />
                ...
                <DataRow
                  ...
                  value={residenceDocument?.numero}
                />

                <DataRow
                  label="Expiration du titre"
                  value={
                    residenceDocument?.date_expiration
                      ? formatDate(
                          residenceDocument.date_expiration
                        )
                      : "Non renseignée"
...
function DocumentAlert({ profile }) {
  const document = getClosestExpiringDocument(profile);
  ...
  if (!document) {
    return (
      <article className="profile-document-alert is-clear">
        ...
        <p>
          Aucun document administratif arrivant
...
function toFormData(data = {}) {
  return {
    ...
    contact_urgence_telephone:
      ...
      data.contact_urgence?.telephone ||
      "",
  };
}
...
function getDocuments(profile) {
  return Array.isArray(profile?.documents)
    ? profile.documents
    : [];
}

function getIdentityDocument(profile) {
  return (
    getDocuments(profile).find((document) =>
      ["CNI", "PASSEPORT"].includes(
...
function getResidenceDocument(profile) {
  return (
    getDocuments(profile).find(
      (document) =>
        document.type_document === "TITRE_SEJOUR"
    ) || null
  );
}
...
function getClosestExpiringDocument(profile) {
  const candidates = getDocuments(profile)
    .filter((document) => document.date_expiration)
    .map((document) => ({
      ...document,
    ...
    .filter((document) => document.days <= 90)
